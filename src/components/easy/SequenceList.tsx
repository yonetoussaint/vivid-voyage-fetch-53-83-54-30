    ...
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          ...
          {/* Denomination summary - similar to grid display */}
          {sequence.breakdown && (
...
// Alternative: Card-style with color bar like denominations in grid
const SequenceItemCard = ({ 
  ...
  return (
    <div className={`rounded-lg border border-gray-200 overflow-hidden ${isEditing ? 'ring-2 ring-amber-300' : ''}`}>
      <div className="flex">
        {/* Color bar on left side */}
        <div className={`${sequenceColor} w-1.5 flex-shrink-0`}></div>
        
        <div className="p-3 flex-1">
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-3 min-w-0 flex-1">
              {/* Denomination chip - exactly like in MoneyCounterGrid */}
              {primaryDenom && (
                <div className={`${sequenceColor} px-2 py-1 rounded-md flex-shrink-0 flex items-center justify-center`}>
                  <span className="text-white font-bold text-xs">{primaryDenom.value}</span>
                </div>
              )}
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {Object.entries(sequence.breakdown)
                      ...
                      .map(([denomValue, count]) => (
                        <div 
                          key={denomValue} 
                          className="inline-flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded text-xs"
                        >
                          <span className="font-medium text-gray-700">{denomValue}</span>
                          <span className="text-gray-600">×{count}</span>
                        </div>
                      ))}
                    {Object.entries(sequence.breakdown)
                      .filter(([_, value]) => value && parseFloat(value) > 0).length > 3 && (
                      <span className="text-xs text-gray-500">+{Object.entries(sequence.breakdown)
                        .filter(([_, value]) => value && parseFloat(value) > 0).length - 3} more</span>
                    )}
                  </div>
                )}
              </div>
