...
const SellerLayout: React.FC<SellerLayoutProps> = ({  
  ...
  const normalTabsRef = useRef<HTMLDivElement>(null); // New ref for normal tabs
  ...
  const [resetNormalTabsScroll, setResetNormalTabsScroll] = useState(false); // New state
  ...
  const handleTabChange = (tabId: string) => {  
    ...
    if (item) {
      ...
      if (tabId === 'products') {
        ...
        setResetNormalTabsScroll(true); // Trigger scroll reset for normal tabs
      } else {
  ...
  // ===== RESET NORMAL TABS SCROLL =====
  useEffect(() => {
    if (resetNormalTabsScroll && normalTabsRef.current) {
      const scrollContainer = normalTabsRef.current.querySelector('[class*="overflow-x-auto"]');
      if (scrollContainer) {
        scrollContainer.scrollLeft = 0;
      }
      setResetNormalTabsScroll(false);
    }
  }, [resetNormalTabsScroll]);
    ...
    <div className="min-h-screen bg-white">  
      ...
      >  
        <div className="relative">  
          {/* Normal Tabs */}  
          <div  
            ref={normalTabsRef} // Add ref to normal tabs
