...
export default function XProfile() {
  ...
  const visibleContacts = showAllContacts ? contactMethods : contactMethods.slice(0, 4);
  ...
  return (
    <div className="bg-white text-gray-900 min-h-screen w-full max-w-[600px] mx-auto">
      ...
      <div className="px-4 pt-2">
        ...
        <div className="grid grid-cols-4 gap-2 mb-3 p-2 bg-gray-50 rounded-lg border border-gray-200">
        ...
        {/* COMPACT Contact Section - Integrated into Profile */}
        <div className="mb-4">
          {/* Contact Buttons Row - COMPACT */}
          <div className="flex flex-wrap gap-2 mb-2">
            {visibleContacts.map((contact) => {
              ...
              return (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-full text-xs font-medium text-gray-700 transition-colors active:scale-95"
                >
                  <div className="flex-shrink-0">
                    {typeof Icon === 'function' ? <Icon /> : <Icon size={12} />}
                  </div>
            ...
            {contactMethods.length > 4 && (
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 rounded-full text-xs font-medium text-gray-700 transition-colors"
              >
                {showAllContacts ? 'Less' : `+${contactMethods.length - 4}`}
              </button>
            )}
          </div>
          ...
          <div className="flex gap-2">
            <a
              href={`tel:${sellerData.contacts.phone}`}
              className="flex-1 py-2 bg-blue-600 text-white rounded-lg font-semibold text-xs hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-1.5"
            >
              <Phone size={14} />
              Call Now
            </a>
            <a
              href={`https://wa.me/${sellerData.contacts.whatsapp.replace(/\D/g, '')}`}
              className="flex-1 py-2 bg-green-600 text-white rounded-lg font-semibold text-xs hover:bg-green-700 active:scale-95 transition-all flex items-center justify-center gap-1.5"
            >
          ...
          <div className="mt-2 px-2 py-1.5 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-yellow-800">Responds within 30 min</span>
            </div>
          </div>
        </div>
      </div>
