...
export default function BuyerSellerChat() {
  ...
  // Determine which cards to show in the progress flow
  const showStep1 = ["accepted", "payment_pending", "delivery_pending", "completed", "refunded"].includes(currentOrder.status)
  const showStep2 = ["payment_pending", "delivery_pending", "completed", "refunded"].includes(currentOrder.status)
  const showStep3 = ["delivery_pending", "completed", "refunded"].includes(currentOrder.status)
  const showCompleted = currentOrder.status === "completed" && currentOrder.receipt
  const showRefunded = currentOrder.status === "refunded"
    ...
    <div className="w-full h-screen flex flex-col overflow-hidden relative transition-colors duration-300">
      <div className="flex-1 flex flex-col min-h-0 bg-background text-foreground">
        ...
        <div ref={chatContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-3 py-3 scroll-smooth">
          ...
          {/* Order Cards with Stable Progress Bar */}
          <div className="space-y-2 mb-3">
            {/* Seller's Offer Card - Always shown as Step 0 */}
                ...
                <div className="bg-card border border-border rounded-xl p-4 shadow-sm">
                  ...
                  {currentOrder.status === "accepted" && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-2 mb-2">
                      <div className="flex items-center gap-2 text-xs text-emerald-700">
                        ...
                        <span>Offer accepted • {currentOrder.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </div>
                  )}
                  {currentOrder.status === "payment_pending" && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 mb-2">
                      <div className="flex items-center gap-2 text-xs text-amber-700">
                        ...
                        <span>Payment pending • Awaiting PIN</span>
                      </div>
                    </div>
                  )}
                  {currentOrder.status === "delivery_pending" && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-2">
                      <div className="flex items-center gap-2 text-xs text-blue-700">
                        ...
                        <span>Awaiting delivery • ${currentOrder.total} held securely</span>
                      </div>
                    </div>
                  )}
                  {currentOrder.status === "completed" && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-2 mb-2">
                      <div className="flex items-center gap-2 text-xs text-emerald-700">
                        <Check className="w-3 h-3" />
                        <span>Order completed • ${currentOrder.total} paid</span>
                      </div>
                    </div>
                  )}
                  {currentOrder.status === "refunded" && (
                    <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 mb-2">
                      <div className="flex items-center gap-2 text-xs text-slate-700">
                        <X className="w-3 h-3" />
                        <span>Order refunded • ${currentOrder.total} returned</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Progress Flow Cards - Only shown if offer is accepted or beyond */}
            {currentOrder.status !== "offer" && (
